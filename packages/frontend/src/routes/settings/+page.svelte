<script lang="ts">
	...
	onMount(async () => {
		...
		// Calendar is connected via login — load calendars
		...
		{:else}
			...
			<section class="bg-white rounded-lg shadow mb-6">
				...
				<div class="p-6">
							...
							<div class="flex items-center p-3 bg-green-50 rounded-md">
								<div class="flex items-center gap-2">
									...
									<span class="text-green-800">
										Calendar connected as {$calendarStore.authStatus?.email || 'Unknown'}
									</span>
								</div>
							</div>
							...
							{#if availableCalendars.length > 0}
								<div>
									...
									<p class="text-sm text-gray-500 mb-3">
										Choose which calendars to check for travel events.
									</p>
					...
					{:else}
						<p class="text-gray-500">Calendar access was not granted during sign-in. Sign out and sign in again to grant calendar access.</p>
					{/if}
				</div>
			</section>
